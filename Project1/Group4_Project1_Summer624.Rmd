...
```{r split-test-train}
sample_index = sort(sample(nrow(S01_ts_Var01_diff), nrow(S01_ts_Var01_diff)*.80))
...
```{r auto-arima-var-01}
S01_ts_Var01 <- ts(select(train,Var01_diff))

fit_var01_auto <- auto.arima(S01_ts_Var01_diff)
fit_var01_auto
```

```{r auto-arima-residuals-var01}
checkresiduals(fit_var01_auto)
```
### ETS - Variable 01 Differentiated
```{r ets-var01}
fit_var01_ets <- ets(S01_ts_Var01_diff)
fit_var01_ets
```

```{r ets-residuals-var01}
checkresiduals(fit_var01_ets)
```
### Auto Arima - Variable 02 Differentiated
```{r  auto-arima-var-02}
S01_ts_Var02_diff <- ts(select(train,Var02_diff))

fit_var02_auto <- auto.arima(S01_ts_Var02_diff)
fit_var02_auto
```
...
```{r ets-var02}
fit_var02_ets <- ets(S01_ts_Var02_diff)
fit_var02_ets
```

```{r ets-residuals-var02}
checkresiduals(fit_var02_ets)
```

## Forecasting Data
For **Variable01** & **Variable02** we can see that the *auto.arima* models are more accurate since the AIC values are lower than the ETS models.
